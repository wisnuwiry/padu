//! Padu-specific post-processing of [`merman`]-produced SVGs.
//!
//! Each submodule is a specific pass that tweaks the SVG event iterator in a particular way.
//!
//! Events are always produced and consumed as [`Event`]s with a short lifetime.
//! [`Event<'a>`] is backed internally by a [`Cow<'a, [u8]>`](std::borrow::Cow),
//! so text inside an [`Event`] can be mutated without lifetime issues, but without
//! forcing a new [`String`] allocation each time.
//!
...
pub(super) fn postprocess(svg: &str, theme: &MermaidTheme) -> Result<String> {
    ...
    // The remaining passes are Padu-specific theme and accent adjustments.
