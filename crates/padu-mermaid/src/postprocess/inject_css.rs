...
//! #padu-merman-1 .node rect { fill: #89b4fa !important; }
//! /* ... theme rules ... */
//! </style>
//! ```
...
use super::util::text_color_for_background;
...
fn mindmap_section_css(theme: &MermaidTheme) -> String {
    ...
    let fills: [gpui::Hsla; 8] = theme.git_branch_colors.map(|c| tint_fill(theme, c));
    let mut css = String::with_capacity(5_400);

    let emit = |css: &mut String, selector: &str, color: &str, fill: gpui::Hsla| {
        let fill_css = crate::css_color(fill);
        let txt = tint_text(fill);
        ...
        write!(
            ...
            "{selector} rect, {selector} path, {selector} circle, {selector} polygon \
             {{ fill: {fill_css} !important; stroke: {color} !important; }}\n\
    ...
    emit(&mut css, ".section-root.section--1", &colors[0], fills[0]);
    emit(&mut css, ".section--1", &colors[1], fills[1]);
    for (i, selector) in MINDMAP_SECTION_SELECTORS.iter().enumerate() {
        let ci = 2 + (i % 6);
        emit(&mut css, selector, &colors[ci], fills[ci]);
    }
    css
}
...
fn git_branch_css(theme: &MermaidTheme) -> String {
    ...
    for i in 0..8 {
        ...
        let label_fill = crate::css_color(tint_fill(theme, theme.git_branch_colors[i]));
        write!(
            css,
...
/// Opacity of a hue blended over the canvas to form a fill that sits behind
/// shaped text. Dark mode needs a deeper tint: a 15% wash over a near-black
/// canvas stays pale under light text (unreadable); light mode keeps the airy
/// wash under dark text.
fn fill_tint_opacity(dark_mode: bool) -> f32 {
    if dark_mode { 0.35 } else { 0.15 }
}

/// A hue turned into a text-backing fill for the active appearance.
fn tint_fill(theme: &MermaidTheme, hue: gpui::Hsla) -> gpui::Hsla {
    blend_over_background(hue, theme.background, fill_tint_opacity(theme.dark_mode))
}

/// Text color with guaranteed contrast on a tinted fill. Labels inside tinted
/// boxes use this instead of the theme text color, which is only guaranteed
/// against the canvas — not against an arbitrary hue wash.
fn tint_text(fill: gpui::Hsla) -> String {
    crate::css_color(text_color_for_background(fill))
}
...
fn accent_css(theme: &MermaidTheme) -> String {
    ...
    for (i, accent) in theme.accent_colors.iter().enumerate() {
        ...
        let fill = tint_fill(theme, accent.background);
        let fill_css = crate::css_color(fill);
        // Labels sit on the tinted fill, so they take a contrasting color
        // rather than the theme text (which is only guaranteed on canvas).
        let text = tint_text(fill);
        let class = format!(".padu-accent-{i}");
        write!(
            css,
            "{class} rect, {class} path, {class} circle, {class} polygon, {class} ellipse, \
             ...
             {{ fill: {fill_css} !important; stroke: {stroke} !important; }}\n\
...
fn timeline_css(theme: &MermaidTheme) -> String {
    ...
    for i in 0..8 {
        let c = crate::css_color(theme.git_branch_colors[i]);
        let fill = crate::css_color(tint_fill(theme, theme.git_branch_colors[i]));
        write!(
            css,
    ...
    for i in 0..4 {
        ...
        let fill = tint_fill(theme, theme.git_branch_colors[i % 8]);
        let fill_css = crate::css_color(fill);
        // Task labels sit on the tinted fill: contrasting text, not canvas text.
        let task_text = tint_text(fill);
        write!(
            css,
            ".section{i} {{ fill: {fill_css} !important; }}\n\
             .task{i} {{ fill: {fill_css} !important; stroke: {c} !important; }}\n\
             .taskText{i} {{ fill: {task_text} !important; }}\n\
...
fn build_injected_css(theme: &MermaidTheme, svg_id: &str) -> String {
    ...
    let raw_css = format!(
        ...
        .relationshipLabelBox {{ fill: {primary} !important; opacity: 0.7; background-color: {primary} !important; }}
        .labelBkg {{ background-color: {primary} !important; }}
...
mod tests {
    ...
    /// Labels inside tinted boxes must stay readable in both appearances.
    /// Guards the "white text on pale wash" failure: every accent hue paired
    /// with its [`tint_text`] on [`tint_fill`] must clear WCAG AA.
    #[test]
    fn tinted_fills_keep_text_contrast_in_both_modes() {
        use super::super::util::{contrast_ratio_between, relative_luminance};

        let hues = [
            rgb(0x8B5CF6).into(), // accent violet
            rgb(0x62C987).into(), // success green
            rgb(0xE0B36A).into(), // warning amber
            rgb(0xE2726A).into(), // danger red
        ];
        for dark_mode in [false, true] {
            ...
            for hue in hues {
                let fill = tint_fill(&theme, hue);
                let text = text_color_for_background(fill);
                let ratio = contrast_ratio_between(
                    relative_luminance(gpui::Rgba::from(fill)),
                    relative_luminance(gpui::Rgba::from(text)),
                );
                assert!(
                    ratio >= 4.5,
                    "hue {hue:?} on dark_mode={dark_mode} only reaches {ratio:.2}:1"
                );
            }
        }
    }
