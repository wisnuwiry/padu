...
//!
//! The root `<svg>` background is dropped: the diagram renders inside Padu's
//! own bordered card, so a painted canvas behind it is redundant — transparency
//! lets the card show through in both appearances.
//!
...
//! ```
//!
//! Rounds every `<rect>` (nodes, clusters, pills) for the flat modern look.
//! Radii larger than half the box are clamped per the SVG spec, so thin
//! dividers simply become pills.
...
/// Corner radius for node rects. usvg/resvg ignore `rx`/`ry` in CSS, so this
/// is applied as attributes (see [`round_rect`]).
const NODE_RADIUS: &str = "8";

/// Adds an attribute to an element, preserving the rest.
fn add_attr<'a>(e: &BytesStart<'_>, key: &str, value: &str) -> Result<BytesStart<'a>> {
    let name = e.name();
    let tag = std::str::from_utf8(name.as_ref())?;
    let mut new_elem = BytesStart::new(tag.to_owned());
    for attr in e.attributes() {
...
fn has_attr(e: &BytesStart<'_>, attr_name: &[u8]) -> bool {
...
/// Applies the node corner radius to a `<rect>` that lacks its own.
fn round_rect<'a>(e: &BytesStart<'_>) -> Result<BytesStart<'a>> {
    let mut new_elem = if has_attr(e, b"rx") {
        let name = e.name();
        let tag = std::str::from_utf8(name.as_ref())?;
        ...
        add_attr(e, "rx", NODE_RADIUS)?
    };
    if !has_attr(&new_elem, b"ry") {
        let name = new_elem.name();
        let tag = std::str::from_utf8(name.as_ref())?.to_owned();
        let mut elem = BytesStart::new(tag);
        for attr in new_elem.attributes() {
            elem.push_attribute(attr?);
        }
        elem.push_attribute(("ry", NODE_RADIUS));
        new_elem = elem;
    }
    Ok(new_elem)
}
/// Drops the `background-color` declaration from a `style` attribute, if any.
/// The diagram renders inside Padu's own card, so a painted canvas behind it
/// is redundant — transparency lets the card surface show through.
fn strip_background_style<'a>(style: &'a str) -> Cow<'a, str> {
    const PROPERTY: &str = "background-color:";

    let Some(property_start) = style.find(PROPERTY) else {
        return Cow::Borrowed(style);
    };
    // Also swallow one adjacent `;` and surrounding whitespace so no empty
    // declaration is left behind.
    let mut value_end = style[property_start..]
        .find(';')
        .map_or(style.len(), |offset| property_start + offset + 1);
    while value_end < style.len() && style[value_end..].starts_with(' ') {
        value_end += 1;
    }
    let mut start = property_start;
    while start > 0 && style[..start].ends_with(' ') {
        start -= 1;
    }
    let mut stripped = String::with_capacity(style.len().saturating_sub(value_end - start));
    stripped.push_str(style[..start].trim_end());
    if !stripped.is_empty() {
        stripped.push(' ');
    }
    stripped.push_str(style[value_end..].trim_start());
...
impl<'a, I: Iterator<Item = Result<Event<'a>>>> ElementFixup<I> {
    fn rewrite_svg_style(&self, e: &BytesStart<'_>) -> Result<Option<BytesStart<'a>>> {
        ...
        let new_style = strip_background_style(&style);
        ...
        match &event {
            ...
            Event::Start(e) | Event::Empty(e) if e.name().as_ref() == b"rect" => {
                if is_bad_rect(e)? {
                    ...
                    Ok(Some(rewrap(&event, round_rect(e)?)))
                }
            }
