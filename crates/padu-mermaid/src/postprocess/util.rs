...
pub(crate) fn relative_luminance(c: Rgba) -> f32 {
    0.2126 * srgb_to_linear(c.r) + 0.7152 * srgb_to_linear(c.g) + 0.0722 * srgb_to_linear(c.b)
}

pub(crate) fn contrast_ratio_between(luminance_a: f32, luminance_b: f32) -> f32 {
...
mod tests {
    ...
    fn dark_background_gets_light_text_and_vice_versa() {
        let dark_bg: Hsla = gpui::rgb(0x111111).into();
        let light_bg: Hsla = gpui::rgb(0xFFFFFF).into();
        let light_text = text_color_for_background(dark_bg);
        let dark_text = text_color_for_background(light_bg);
        let light_lum = relative_luminance(Rgba::from(light_text));
        let dark_lum = relative_luminance(Rgba::from(dark_text));
        assert!(
            light_lum > 0.5,
            "dark bg should get light text, got luminance {light_lum}"
        );
        assert!(
            dark_lum < 0.5,
            "light bg should get dark text, got luminance {dark_lum}"
        );
        let ratio = contrast_ratio_between(relative_luminance(Rgba::from(dark_bg)), light_lum);
        assert!(ratio >= 4.5, "contrast ratio {ratio} < 4.5");
    }
}
