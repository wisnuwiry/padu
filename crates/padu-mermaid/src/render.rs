...
pub(super) fn render_mermaid(source: &str, theme: &MermaidTheme) -> Result<String> {
    ...
    let diagram_id = format!("padu-merman-{id}");
    ...
    // Apply merman's raster-safe pipeline before Padu-specific styling. The
    ...
    // Padu also strips merman's existing `!important` declarations before
...
mod tests {
    ...
    #[test]
    fn app_font_family_reaches_labels() {
        let theme = MermaidTheme {
            font_family: ".SystemUIFont".into(),
            ..MermaidTheme::default()
        };
        let svg = render_mermaid("flowchart LR\n  A --> B", &theme).expect("render failed");
        assert!(
            svg.contains(".SystemUIFont"),
            "diagram labels must carry the app face"
        );
    }
