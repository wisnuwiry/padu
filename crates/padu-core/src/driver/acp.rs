...
pub(crate) fn launch_for(
    ...
    match provider {
        ProviderKind::Agy => super::acp_agy::agy_launch(agy_session_temp),
...
pub(crate) fn sdk_agent_with_stderr_callback(
    binary: &Path,
    cwd: &Path,
    ...
    let result = Client
        ...
        .connect_with(agent, async move |connection: ConnectionTo<Agent>| {
            ...
            let initialize = connection
                ...
                .await?;
            if provider == ProviderKind::Agy {
                super::acp_agy::authenticate_agy_connection(&connection, &initialize).await?
            }
                        ...
                        if let Some(pending) = pending_user_inputs.lock().remove(&request_id) {
                            match pending {
                                ...
                                PendingAcpUserInput::Permission { request, responder } => {
                                    let response =
                                        super::acp_agy::agy_user_input_response(&request, &answers);
                                    let _ = responder.respond(response);
                                }
                            }
                        }
...
pub(crate) fn session_config_select_values(option: &SessionConfigOption) -> Vec<&str> {
...
pub(crate) fn find_config_option(
...
async fn apply_model(
    ...
    let model = if provider == ProviderKind::Agy {
        resolved_model_storage =
            super::acp_agy::resolve_agy_model_id(model, reasoning_effort, config_options);
...
fn send_prompt(
    ...
    let registered = sent.on_receiving_result(async move |result| {
        if settle_prompt_request(&callback_requests, &callback_request_id) {
            super::acp_agy::maybe_inline_agy_plan(
                provider,
                &mut stream_state.lock().agy,
                &callback_events,
            );
...
fn handle_permission_request(
    ...
    if provider == ProviderKind::Agy && super::acp_agy::is_agy_question_request(&params, &request) {
        let questions = super::acp_agy::agy_user_input_questions(&params, &request);
        if questions.is_empty() {
            let _ = responder.respond(RequestPermissionResponse::new(
                RequestPermissionOutcome::Cancelled,
            ));
...
pub(crate) fn handle_session_update(
    ...
    match kind {
        Some("agent_message_chunk") => {
            ...
            {
                if provider == ProviderKind::Agy {
                    state.agy.observe_text_chunk(text);
                }
        ...
        Some("tool_call" | "tool_call_update") => {
            if provider == ProviderKind::Agy {
                super::acp_agy::extract_agy_tool_plan(&update, &mut state.agy);
            }
...
#[derive(Default)]
pub(crate) struct AcpStreamState {
