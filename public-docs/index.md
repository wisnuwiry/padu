...
The desktop app bundles its own lightweight daemon and starts it automatically on a loopback port. No separate server install or cloud account is required.
...
```bash
# Start the standalone daemon on loopback port 34123
padu-daemon --bind 127.0.0.1:34123

# Or allow remote connections over private VPN / Tailscale
padu-daemon --bind 100.101.102.103:34123 --allow-non-loopback
```
...
- [Connectivity](/docs/connectivity) — Connect remote web or mobile clients via LAN, Tailscale, Cloudflare Tunnel, or an SSH relay.
